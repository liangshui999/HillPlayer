...
public class ModelLocalMusicList implements IModelLocalMusicList {

    private static String TAG = "ModelLocalMusicList";

    private Context mContext;

    private PLocalMusicList mPLocalMusicList;

    private ContentResolver mContentResolver;

    public ModelLocalMusicList(PLocalMusicList mPLocalMusicList) {
        TAG = getClass().getSimpleName();
        this.mPLocalMusicList = mPLocalMusicList;
        mContext = MyAppliacation.getContext();
        mContentResolver = mContext.getContentResolver();
    }

    @Override
    public void fetchMusicList(Callback<List<Music>> callback) {
        QueryTask queryTask = new QueryTask(callback);
        queryTask.executeOnExecutor(QueryTask.THREAD_POOL_EXECUTOR,MediaStore.Audio.Media.EXTERNAL_CONTENT_URI);
    }
